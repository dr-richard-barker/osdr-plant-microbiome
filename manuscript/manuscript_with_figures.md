...
### Inferring guilds — and distinguishing plant pathogens from human pathogens

A central operational question for space agriculture is which community members promote plant growth and which threaten health — but "threaten health" conflates two fundamentally different risks that must be kept separate. A **plant pathogen** (phytopathogen such as *Ralstonia solanacearum*, cause of bacterial wilt) threatens the **crop** and its yield; a **human/clinical pathogen** (opportunist such as *Staphylococcus* or *Cutibacterium*) threatens the **crew** as a food-safety hazard. These guilds co-occur in the same growth hardware yet attack different hosts and demand different mitigations (phytosanitary control versus food-handling protocols). We therefore implemented a **multiclass semi-supervised guild-inference engine** that treats beneficial, plant-pathogen and human-pathogen as distinct classes, fusing four independent evidence streams (Methods): (i) curated genus-level priors for each guild, with genuinely ambiguous genera left deliberately unlabelled; (ii) *guild-specific* ecological trait features — a human-swab (built-environment) association index for the human-pathogen axis, and living-plant-tissue and root/vascular association for the plant-pathogen axis, alongside niche breadth, spaceflight response and prevalence; (iii) guilt-by-association computed separately toward each guild on a Spearman co-occurrence network; and (iv) multiclass label spreading returning P(beneficial)/P(plant-pathogen)/P(human-pathogen), with a Random Forest¹⁶ providing interpretable feature importance.

The engine reports **two separate risk scores** — a plant-pathogen (crop) risk and a human-pathogen (food-safety) risk — rather than a single conflated axis (Fig. 5). It placed the phytopathogens *Ralstonia* and *Curtobacterium* on the plant-pathogen axis (high crop risk, low crew risk) and the human-associated *Staphylococcus*, *Cutibacterium*, *Acinetobacter*, *Stenotrophomonas* and *Enterobacter* on the orthogonal human-pathogen axis. Critically, propagation extended beyond the seed labels: the *deliberately unlabelled* *Pseudomonas* was drawn toward the human-opportunist axis by its built-environment association, while *Pantoea* emerged as a genuinely mixed case (P(beneficial) ≈ 0.58, P(human-pathogen) ≈ 0.41), correctly reflecting its dual ecology. Twelve genera were called beneficial (including *Rhizobium*, *Bacillus*, *Paenibacillus*, *Streptomyces*, *Methylobacterium* and *Sphingomonas*), two plant-pathogen and six human-pathogen. The orthogonality of the two risk axes (Fig. 5a) is the key result: it prevents a crop-yield threat from being mistaken for a crew-health threat, and vice versa.

![Figure 5](../figures/2026-07-02/Fig5_guild_scores.png)

**Fig. 5 | Multiclass machine-learning inference of microbial ecological guilds, separating plant from human pathogens. (a)** Each reference genus positioned by its human-pathogen risk (*x*; food-safety / crew-health) and its plant-pathogen risk (*y*; crop-yield); point size is proportional to prevalence and colour denotes the guild call (green, beneficial; orange, plant pathogen; red, human pathogen; grey, uncertain). The two risks are deliberately plotted as **orthogonal axes**: phytopathogens (*Curtobacterium*, *Ralstonia*) occupy the upper-left (crop risk only), clinical opportunists the lower-right (crew risk only), and beneficials the origin. Small deterministic jitter separates co-located low-risk taxa. **(b)** Per-genus stacked guild-probability bars — P(beneficial) (green) + P(plant pathogen) (orange) + P(human pathogen) (red) sum to one — making mixed cases (e.g. *Pantoea*) and confident calls equally legible. Scores use illustrative abundances and constitute a decision-support prior, not a phytosanitary or clinical determination; the method runs unchanged on primary OSDR feature tables (Methods).
...
**Fig. 6 | The plant-microbiome corpus is a moving target, motivating the living-dashboard design.** Cumulative number of plant-associated microbiome studies in OSDR, plotted from the **real public-release dates** retrieved from the OSDR API (`data/registry/release_dates.csv`): three VEG-03 leafy-green studies were released together in June 2024 (OSD-412/413/414), the Chile-pepper study in September 2025 (OSD-772), and the tomato and multi-species studies in May 2026 (OSD-766, OSD-773). The star marks this release (6 studies, 2 July 2026). Because the corpus grows — it was smaller a year ago and will be larger a year hence — any single analysis is a dated snapshot; the tool re-derives all results on each build and stamps every figure and database with its snapshot date and per-row provenance, so transient results remain interpretable and reproducible over time.
...
The guild-inference engine is deliberately conservative and modular, and its most important design decision is conceptual rather than algorithmic: it keeps **plant pathogens and human pathogens on separate axes**. These are both real, important and intimately integrated in a closed crop-growth system — the same hardware that hosts a phytopathogen threatening the tomato canopy may also harbour a human opportunist on a contact surface — but they are distinct guilds that endanger different hosts (crop versus crew) and call for different countermeasures (phytosanitary control versus food-safety handling). Collapsing them into one "pathogen" score would be operationally misleading. Beyond that framing, the value of the engine is its architecture: weak supervision plus guild-specific ecological traits plus network guilt-by-association plus multiclass semi-supervised propagation, so that ambiguous genera are classified by evidence rather than by fiat and every call carries an uncertainty (the mixed *Pantoea* call is a feature, not a failure). In production the same pipeline gains power from strain-level resolution and genome-inferred traits: resolving amplicon sequence variants against *axis-appropriate* curated databases — phytopathogen catalogues for the plant axis, clinical/virulence databases for the human axis — replacing Spearman with compositionally robust co-occurrence (SparCC/SPIEC-EASI)¹⁷, and adding predicted functional potential (PICRUSt2¹⁸ or antiSMASH biosynthetic gene clusters). Crucially, no operational flag should be raised on a single study; the multi-study integration described here is exactly what enables a requirement of cross-study reproducibility before any taxon is designated friend or foe — and to which host.
...
### Guild inference
The engine is **multiclass** (beneficial / plant-pathogen / human-pathogen) so the two distinct notions of pathogen are never conflated. Four evidence streams are fused. (1) Curated genus-level priors label selected genera into each guild and leave ambiguous genera unlabelled. (2) *Guild-specific* ecological trait features are engineered from the harmonised data: a human-swab (built-environment) association index for the human-pathogen axis; a living-plant-tissue association and a root/vascular preference for the plant-pathogen axis; plus tissue-distribution Shannon (niche breadth), spaceflight log-fold change, prevalence and abundance moments. (3) A Spearman co-occurrence network (|ρ|≥0.3) yields guilt-by-association scores computed *separately* toward each labelled guild. (4) Multiclass `LabelSpreading` (RBF kernel) propagates the sparse labels over the standardised feature space to return P(beneficial)/P(plant-pathogen)/P(human-pathogen); a balanced Random Forest¹⁶ provides feature importance. **Two separate risk scores** are reported: plant-pathogen risk (P(plant-pathogen) up-weighted by plant-tissue association) and human-pathogen risk (P(human-pathogen) up-weighted by human-swab association). Recommended production upgrades: strain/ASV-level resolution against curated references — phytopathogen catalogues (e.g. PHI-base) for the plant axis and virulence/clinical databases (e.g. VFDB, BacDive) for the human axis — compositional co-occurrence (SparCC¹⁷), genome-inferred function (PICRUSt2¹⁸), distinct response protocols per risk, and a cross-study reproducibility requirement.
